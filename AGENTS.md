...
- The current incompatible simulation protocol is `5`, with app namespace `grid-infinito-lightcycles-v5`, game action `grid-v5`, and directory rendezvous `GRID-INFINITY-PUBLIC-DIRECTORY-V2`. Older builds cannot share simulation state with it; all peers must reload after this protocol update. Invitation fragments remain unchanged.
...
- “Find match” collects public-room advertisements for about 3.2 seconds and broadcasts repeated discovery probes during that window. Public hosts advertise about every 1.15 seconds and also answer targeted discovery probes. Open rooms are ranked strictly by connected human count, then by the fewest remaining human slots, and the busiest candidate is joined so population consolidates instead of fragmenting. AI count is ignored even when the host has added bots. A fresh uniquely identified public room is created only after the full retry window yields no candidate. A stale full-room result automatically resumes matchmaking.
...
1. Run `node tests/regression.mjs` (real inline simulation/network code, headless), parse the inline script with Node (`new Function`), and run `git diff --check`. The regression suite covers exact geometry, tiny loops, external explosions, rubber recovery, revisions, delayed input bursts, quick brake edges, congestion, respawns, and independent host/guest simulation with up to 360 ms RTT plus variable delay. Browser/device checks below require explicit browser-testing authorization; do not present headless simulation as a real Internet playtest.
