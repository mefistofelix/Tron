...
- The camera defaults to a useful chase view that shows the player’s own bike and a broad area around it. All three playable views use the substantially farther framing requested after the second camera-distance pass: Smart chase scales from about 28.4 units back at base speed to 35 units at maximum speed, with a rise of roughly 16.2–20.3 units; Close chase uses about 18.2 units back and 10.4 high; Top view uses about 52 units of height and 7.5 units of rear offset. Chase targets look about 7.2 units ahead. The three modes remain distinct while every one shows materially more of the arena.
...
- “Rubber” allows a brief, visibly tense approach to a wall before the crash. Its stress accumulation is deliberately brisk—roughly 15% faster than the earlier baseline—so a rider cannot remain pressed against a trail. Stress belongs to the exact segment currently blocking the rider: encountering a different segment resets it, and clear forward travel recovers it quickly at about `1.15/s`. Old contact with an unrelated wall must never cause a later apparently random crash. Rubber must not permit tunneling or crossing.
...
- Simulation remains host-authoritative, but human controls are local-first. A guest immediately predicts each sequenced 90-degree turn and its temporary trail, then sends the intent plus observed position and direction. The host validates it afterward, applies bounded latency compensation by rewinding along the current active segment and replaying the covered distance after the turn, and acknowledges the processed sequence in snapshots. The guest removes acknowledged predicted geometry and smoothly reconciles to authoritative motion; a delayed network round trip must never make the visible turn wait for the host.
...
- Every guest turn carries a monotonically increasing sequence, observed point, and pre-turn direction. Rapid in-flight guest turns remain an ordered bounded queue rather than overwriting one another. Host snapshots return the last accepted sequence per cycle, and a finalized local wall carries the originating input sequence so its temporary predicted copy can be removed immediately without a double trail. Prediction is presentation-only: it never enters authoritative collision state, scoring, or persistent walls, and is discarded on initialization, death, respawn, room change, host loss, or a stale unacknowledged input timeout.
